{ -----------------------------------------------------------------------------
  ...
  Date:      03-Jun-2005
  Purpose:   Generic Dialog for Editing Collections
  History:
  ---------------------------------------------------------------------------- }
...
function EditCollection(ACollection: TCollection;
  ...
  ItemEditFunction: TItemEditFunction; AHelpContext: Integer): Boolean;

function SelectFromCollection(ACollection: TCollection;
  ItemClass: TCollectionItemClass; ACaption: string;
  ItemEditFunction: TItemEditFunction; AHelpContext: Integer;
  var SelectedIndex: Integer; ADialogType: TCEDialogType = cetSelect): Boolean;
...
begin
  Result := False;
  SelectedIndex := -1;
  if not(Assigned(ACollection) and Assigned(ItemEditFunction)) then
    Exit;
...
begin
  ModifyBtn.Enabled := (ItemList.Items.Count > 0) and (ItemList.ItemIndex >= 0);
...
begin
  if FItemEdit(FCollection.Add) then
  begin
    UpdateList;
...
begin
  ...
  if (Index >= 0) and
    FItemEdit(FCollection.FindItemID(NativeInt(ItemList.Items.Objects[Index])))
  then
  begin
    UpdateList;
...
begin
  ...
  begin
    FCollection.Delete(FCollection.FindItemID(NativeInt(ItemList.Items.Objects
      [ItemList.ItemIndex])).Index);
    UpdateList;
  end;
end;
...
begin
  CanClose := (FDialogType = cetEdit) or (ModalResult = mrCancel) or
...
begin
  Index := ItemList.ItemIndex;
  if InRange(Index, 0, ItemList.Count - 2) then
  begin
