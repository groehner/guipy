...
begin
  ...
  FCurrentPPI := ADockSite.CurrentPPI;
...
function TJvDockTree.PPIScale(Value: Integer): Integer;
begin
...
begin
  Result := PPIScale(FBorderWidth);
end;

function TJvDockTree.GetDockSplitterWidth: Integer;
begin
  Result := PPIScale(FSplitterWidth);
end;
...
procedure TJvDockTree.DrawDockGrabber(Control: TWinControl;
  ...
  begin
    ...
    begin
      ...
      StyleServices.DrawElement(Canvas.Handle, Details, Rect(Left, Top,
        Left + GrabberSize - PPIScale(2), Top + GrabberSize - PPIScale(2)));
    end
    else
      {$ENDIF JVCLThemesEnabled}
      {This is the grabber's Close button if one should be drawn.  }
      DrawFrameControl(Canvas.Handle, Rect(Left, Top, Left + GrabberSize - PPIScale(2),
        Top + GrabberSize - PPIScale(2)), DFC_CAPTION, DFCS_CAPTIONCLOSE);
  end;
  ...
  begin
    ...
    Canvas.LineTo(Right, Top - PPIScale(1));
  end;
  ...
  case GrabbersPosition of
    gpLeft:
      begin
        ...
        DrawCloseButton(ARect.Left + BorderWidth + BorderWidth + PPIScale(1), ARect.Top + BorderWidth + BorderWidth + PPIScale(1));
        if FGrabberShowLines then
        begin
          DrawGrabberLine(ARect.Left + BorderWidth + PPIScale(3), ARect.Top + GrabberSize + BorderWidth + PPIScale(1),
            ARect.Left + BorderWidth + PPIScale(5), ARect.Bottom + BorderWidth - PPIScale(2));
          DrawGrabberLine(ARect.Left + BorderWidth + PPIScale(6), ARect.Top + GrabberSize + BorderWidth + PPIScale(1),
            ARect.Left + BorderWidth + PPIScale(8), ARect.Bottom + BorderWidth - PPIScale(2));
        end;
      ...
      begin
        ...
        begin { draw only if color is given }
          ...
          begin
            ...
            Canvas.Pen.Style := psClear;
          Canvas.Rectangle(ARect.Left, ARect.Top, ARect.Right, ARect.Top + GrabberSize + PPIScale(2));
        end;

        DrawCloseButton(ARect.Right - GrabberSize + BorderWidth + PPIScale(1), ARect.Top + BorderWidth + PPIScale(1));
        if FGrabberShowLines then
        begin
          DrawGrabberLine(ARect.Left + BorderWidth + PPIScale(4), ARect.Top + BorderWidth + BorderWidth + PPIScale(3),
            ARect.Right - GrabberSize + BorderWidth - PPIScale(4), ARect.Top + BorderWidth + PPIScale(5));
          DrawGrabberLine(ARect.Left + BorderWidth + PPIScale(4), ARect.Top + BorderWidth + BorderWidth + PPIScale(6),
            ARect.Right - GrabberSize + BorderWidth - PPIScale(4), ARect.Top + BorderWidth + PPIScale(8));
        end;
        ...
        begin
          Canvas.Pen.Color := FGrabberBottomEdgeColor;
          Canvas.Pen.Style := psSolid;
          Canvas.MoveTo(ARect.Left, ARect.Top + GrabberSize - PPIScale(1));
          Canvas.LineTo(ARect.Right, ARect.Top + GrabberSize - PPIScale(1));
        end;
      end;
  end;
end;
