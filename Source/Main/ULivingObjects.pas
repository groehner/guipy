...
function TLivingObjects.InsertObject(const Objectname: string): boolean;
  var i, j: integer; SL: TStringList;
      Py: IPyEngineAndGIL;
...
begin
  for var i:= 0 to SLObjectsAddressName.Count - 1 do
    if SLObjectsAddressName.ValueFromIndex[i] = Objectname then begin
      SLObjectsAddressName.Delete(i);
      break;
    end;
  var i:= SLObjectsNamePath.IndexOfName(Objectname);
  if i > -1 then SLObjectsNamePath.Delete(i);
end;
...
function TLivingObjects.getObjectAttributeValues(const Objectname: string): TStringList;
  var i: integer;
      SL: TStringList;
      Py: IPyEngineAndGIL;
      NS: TBaseNameSpaceItem;
begin
  ...
  NS:= getNodeFromName(Objectname);
  for i:= 0 to NS.ChildCount - 1 do
    if IsDunder(NS.ChildNode[i].Name)
      then break
    else if isAttribute(NS.ChildNode[i]) then
      SL.Add(NS.ChildNode[i].Name + '=' +
             getNameFromValue(NS.ChildNode[i].Value));
...
begin
  ...
  if i > -1
    then Result:= SLObjectsNamePath.ValueFromIndex[i]
    else Result:= '';
end;

function TLivingObjects.isObject(Node: TBaseNameSpaceItem): boolean;
begin
  if Node.isClass or Node.IsDict or Node.IsModule or Node.isFunction or Node.isMethod then
    Exit(false);
...
begin
  Result:= (Node.ObjectType <> 'method') and (Node.ObjectType <> 'method-wrapper');
end;
...
procedure TLivingObjects.makeAllObjects;
  ...
  procedure Add(Prefixname: string; NS: TBaseNameSpaceItem);
    var i: integer; Name, Path: String;
  begin
    i:= 0;
