...
begin
  inherited;
  LBAssociations.ItemHeight := LBAssociations.Height div LBAssociations.
    Items.Count;
  CBReadingOrderA.Caption := #$25C0 + ' ';
  CBReadingOrderB.Caption := #$25B6 + ' ';
  if IsStyledWindowsColorDark then
    FILAssociations := vilConnectionsDark
  else
    FILAssociations := vilConnectionsLight;
end;
...
begin
  ...
  FILAssociations.Draw(ACanvas, 4,
    Rect.Top + (Rect.Height - FILAssociations.Height) div 2, Index);
...
procedure TFAssociation.Init(IsConnecting: Boolean; Conn: TConnection;
  SelectedControls: Integer);
begin
  ClientHeight := 495;
  var
  Height := ((MMultiplicityA.Top + MMultiplicityA.Height) +
    RGRecursivCorner.Top) div 2 + 4;
...
const
  ArrowLeft: string = #$25C0 + ' ';
begin
  var
  Pos := Pos(ArrowLeft, ERelation.Text);
  if CBReadingOrderA.Checked then
  begin
    if Pos = 0 then
...
const
  ArrowRight: string = ' ' + #$25B6;
begin
  var
  Pos := Pos(ArrowRight, ERelation.Text);
  if CBReadingOrderB.Checked then
  begin
    if Pos = 0 then
