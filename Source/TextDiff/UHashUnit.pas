...
function CalcCRC32(Posi: PChar; Length: Integer): DWORD;
begin
  Result := $FFFFFFFF;
  for var I := 0 to Length - 1 do
  begin
    Result := (Result shr 8) xor Table[PByte(Posi)^ xor (Result and $000000FF)];
    Inc(Posi);
  end;
  Result := not Result;
end;
...
begin
  ...
  begin
    ...
    while Int <= Len do
    begin
      if not CharInSet(Line[Int], [#9, #32]) then
      begin
        Str[IntJ] := Line[Int];
        Inc(IntJ);
      end;
    ...
    SetLength(Str, IntJ - 1);
  end;
  if IgnoreCase then
    Str := AnsiLowerCase(Str);
  Result := CalcCRC32(PChar(Str), Length(Str));
end;
